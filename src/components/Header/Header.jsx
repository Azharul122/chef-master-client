...
//    console.log(user?.displayName)
    ...
    return (
        <div>
                ...
                <nav className="bg-white border-gray-200 px-4 lg:px-6 py-2.5 dark:bg-gray-800">
                    <div className="flex flex-wrap justify-between items-center mx-auto max-w-screen-xl">
                        ...
                        <div className="flex items-center lg:order-2">
                            {
                                user?<div className="flex"><img onMouseOver={blockUserDetails} src={user?.photoURL} alt="" className='w-[50px] h-[50px] rounded-full' /></div> :           <Link to={"login"} className="text-gray-800 dark:text-white hover:white focus:ring-4 focus:ring-gray-300 font-medium rounded-lg text-sm md:text-md px-4 lg:px-5 py-2 lg:py-2.5 mr-2 focus:outline-none bg-[#374151]">Log in</Link>
                            }
            ...
            <div className="hidden userInfo absolute bg-black text-white py-5 px-3 right-9 text-center" id='userDetails' onMouseLeave={hideUserDetails} >
                            <p className="text-md mb-4">{user?.displayName}</p>    
                            <Link to={"/"} onClick={handleLogOut} className="text-gray-800 dark:text-white hover:white focus:ring-4 focus:ring-gray-300 font-medium rounded-lg text-sm md:text-md px-4 lg:px-5 py-2 lg:py-2.5 mr-2 focus:outline-none bg-[#374151]">Log Out</Link>
            </div>
        </div>
    );
};
